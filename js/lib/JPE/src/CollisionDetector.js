JPE.declare("CollisionDetector", function(){
	...
	JPE.CollisionDetector = {
			...
			test: function(objA, objB){
				//var root = window.main.pages[window.main.RACE_PAGE].race;

				//var resolveRect = new Rect( root.viewPortRect.x, root.viewPortRect.y, 5000, 5000 );

				if(objA.getFixed() && objB.getFixed()) return;
				//if( resolveRect.contains( objA.curr.x, objA.curr.y) && resolveRect.contains( objA.curr.x, objB.curr.y ) )
			//	{
			...
			testOBBvsOBB: function(ra, rb){
				...
				for(var i = 0; i < 2; i++){
					...
					if(depthA == 0)
					{
						ra.removeCollision(rb);
						rb.removeCollision(ra);
						return false;
					}
			...
			testOBBvsCircle: function(ra, ca){
				...
				for(; i < 2; i++){
					...
					if(depth == 0)
					{
						ra.removeCollision(ca);
						ca.removeCollision(ra);
						return false;
					}
			...
			testCirclevsCircle:function(ca, cb){
				...
				ca.removeCollision(cb);
				cb.removeCollision(ca);
				return false;
			},
			/**
