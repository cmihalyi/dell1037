(function (window) {
	...
	function OEMBuildWheels(main) {
		...
		this.BASE_NAME = 'oem-build-wheels-page';
	...
	OEMBuildWheels.prototype = new Page();

	OEMBuildWheels.prototype.parent = Page.prototype;

	OEMBuildWheels.prototype.create = function() {
		...
		this.wheels.push(Utilities.createCarOption(this.WHEEL_UPPER_LEFT, this, this.main.OEM_BUILD_BRAKES, null, this.main.BUILD_TYPE_OEM).appendTo(this.leftSide));
		this.wheels.push(Utilities.createCarOption(this.WHEEL_UPPER_RIGHT, this, this.main.OEM_BUILD_BRAKES, null, this.main.BUILD_TYPE_OEM).appendTo(this.leftSide));
	...
	OEMBuildWheels.prototype.createWheelStats = function(stats) {
		var instance = this;
	...
	OEMBuildWheels.prototype.activate = function(content) {
		...
		$('#footer-previous-button').addClass("prev-btn").click(function(){
			...
			instance.main.navigateBackward(instance, instance.main.OEM_BUILD_CHASSIS);
		...
		if(content != null){
            $("#main-wrapper-navigation-pane h3").text(content.oem_build_wheels.nav_pane_title);		
		}

		this.carPreview = Utilities.createSmallCarPreview(this.BASE_WHEELS_OPTION, this.WHEELS_PREVIEW, this.main.BUILD_TYPE_OEM, this.main).appendTo(this.rightSide);
	...
	OEMBuildWheels.prototype.distributeContent = function(content) {
		if ( this.root != null )
		{
            var instance = this;
			var rootContent = content;
			var content = content.oem_build_wheels;
			...
			for (i = 0; i <= 1; i++) {
				...
				{
					case "speed":
						this.leftSide.find('span[id|=car-option-'+i+'-mod1-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].speed);
						break;
					case "braking":
						this.leftSide.find('span[id|=car-option-'+i+'-mod1-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].braking);
						break;
					case "durability":
						this.leftSide.find('span[id|=car-option-'+i+'-mod1-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].durability);
						break;
					case "handling":
						this.leftSide.find('span[id|=car-option-'+i+'-mod1-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].handling);
						break;
					default:
						break;
				}
				...
				{
					case "speed":
						this.leftSide.find('span[id|=car-option-'+i+'-mod2-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].speed);
						break;
					case "braking":
						this.leftSide.find('span[id|=car-option-'+i+'-mod2-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].braking);
						break;
					case "durability":
						this.leftSide.find('span[id|=car-option-'+i+'-mod2-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].durability);
						break;
					case "handling":
						this.leftSide.find('span[id|=car-option-'+i+'-mod2-value]').text(this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].handling);
						break;
					default:
						break;
				}
				this.leftSide.find('span[id|=car-option-' + i + '-cost-amount]').text(this.main.CURRENCY_SYMBOL + this.main.parts.wheels[(this.main.buildConfigs["oem"].compatibleWheels[i])-1].cost);
	...
	OEMBuildWheels.prototype.distributeStatContent = function(option, word, section) {
		$('#' + this.BASE_WHEELS_OPTION + "-" +option+"-option-row-title"+section).text(word);
	};
		

	OEMBuildWheels.prototype.attach = function() {
		this.root.appendTo('#main-wrapper-content-body');
	};

	OEMBuildWheels.prototype.detach = function() {
		this.root.detach();
	};

	window.OEMBuildWheels = OEMBuildWheels;
